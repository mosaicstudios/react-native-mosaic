export default class ListHelper {
  ...
  static removeItem(list, selectedItem, key = null) {
    let index = ListHelper.indexOf(list, selectedItem, key);
    if (index === -1) {
      return;
    }
    list.splice(index, 1);
  ...
  static getItemByValue(list, value, key) {
    let index = ListHelper.findItemByValue(list, value, key);
    return index > -1 ? list[index] : null;
  }

  static contains(list, item, key = null) {
    let index = ListHelper.indexOf(list, item, key);
    return index > -1;
  }

  static findAndReplace(list, item) {
    let index = ListHelper.indexOf(list, item, 'id');
    if (index < 0) {
      return list;
    }
    list[index] = item;
